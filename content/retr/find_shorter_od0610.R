...
# ?????????????????????????????????????????????????? add a new level
##### Levels of "access"
# customers  designated destination   emergency        foot     license 
# 1090           1        1723           1           1           9 
# no  permissive      permit     private     unknown         yes 
# 669        1741           3        9589          12         894 
...
## Deleting the broken segment
network_broken <- network[!lengths(st_intersects(network, st_buffer(disaster_nearby, buffer))), ]
### From 12883 obs. to 12736 obs.

## Using the "access" column
network$access <- "no"
network_broken <- network

network_broken$access[!lengths(st_intersects(network_broken, st_buffer(disaster_nearby, buffer)))] <- "yes"
## Using a new level of 9999
# network_broken$MOTORCODE[lengths(st_intersects(network_broken, st_buffer(disaster_nearby, buffer)))] <- 9999
...
## Broken network
net_broken <- weight_streetnet (network_broken, 
                                wt_profile = wts, 
                                type_col = "MOTORCODE", 
                                id_col = "LOCALID",
                                keep_cols = "access") 
...
### Reduce to 6376 obs. #### remove the "access" column
